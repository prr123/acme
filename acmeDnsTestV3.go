...
//	numAcmeDom := len(domains)
	...
	csr.DNSNames = make([]string, numAcmeDom)

	for i:=0; i< numAcmeDom; i++ {
		csr.DNSNames[i] := acmeDomList[i]
	}

/*
	csr, err := x509.CreateCertificateRequest(rand.Reader, req, key)
	if err != nil {	log.Fatalf("CreateCertReq: %v",err)}
	crt, _, err := client.CreateCert(ctx, csr, 90*24*time.Hour, true)
	if err != nil {
...
*/
	...
	url := ord2.FinalizeURL

	derCert, certUrl, err := client.CreateOrderCert(ctx, url, csr, true)
	if err != nil {
		log.Fatalf("CreateOrderCert: %v\n",err)
	}
